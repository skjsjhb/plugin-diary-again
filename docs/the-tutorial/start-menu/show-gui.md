...
每个物品栏都有一个**所有者（Holder）**，一般而言，在插件开发中，物品栏会和其所有者一并创建，因此我们新建一个类，实现 `InventoryHolder` 接口，并在其中创建我们的物品栏：
...
当创建了 `StartMenuInventoryHolder` 类后，稍后就可以通过实例化它来获得一个物品栏，不过让我们先把这个放在一边，等到编写事件处理器时再使用。
