...
现在回到 `build.gradle.kts`，在文件的最后增加下面的内容：
...
这就是第一个插件的全部代码了！事实上我们只是创建了两个新文件，修改了一个已有的文件，并且增加了短短十几行代码，还挺简单，不是吗？虽然你可能还搞不懂这些代码的含义，但那也不要紧，本书的所有剩余内容几乎都是关于“如何编写正确的代码”的。
